...
export default function HomeScreen() {
  ...
  const [defaultModelUri, setDefaultModelUri] = useState<string | null>(null);
  const defaultModelUriRef = React.useRef<string | null>(null);

  // Load local .glb from assets for hero viewer and fallback
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        const asset = Asset.fromModule(require('@/assets/Ring N080111.glb'));
        await asset.downloadAsync();
        const uri = asset.localUri ?? asset.uri ?? null;
        if (uri && !cancelled) {
          defaultModelUriRef.current = uri;
          setDefaultModelUri(uri);
        }
      } catch (e) {
        if (!cancelled) console.warn('Could not load assets/Ring N080111.glb:', e);
      }
    })();
    return () => {
      cancelled = true;
  ...
  const takePicScale = useSharedValue(1);
  const handleTakePhoto = async () => {
    try {
      ...
      if (!result.canceled && result.assets[0]) {
        ...
        takePicScale.value = withSequence(
  ...
  const takePicButtonStyle = useAnimatedStyle(() => ({
    transform: [{ scale: takePicScale.value }],
  }));
  ...
  const handleMorphGem = async () => {
    ...
    try {
      // Backend: replace with your friend's API URL when ready
      const BACKEND_MORPH_URL = process.env.EXPO_PUBLIC_MORPH_API_URL || 'https://your-backend.com/api/morph';
      ...
      const response = await fetch(BACKEND_MORPH_URL, {
        method: 'POST',
        body: formData,
        headers: {
      ...
      if (response.ok) {
        ...
        const modelUrl = data.modelUrl ?? data.model_url ?? data.glb_url;
        if (modelUrl) {
      ...
      // Backend not ready or no modelUrl → show local asset model
      const fallbackUri = defaultModelUriRef.current;
      if (fallbackUri) setMorphedModelUrl(fallbackUri);
      setTimeout(() => scrollViewRef.current?.scrollToEnd({ animated: true }), 300);
    } catch (error) {
      console.error('Error morphing (backend may not be connected):', error);
      const fallbackUri = defaultModelUriRef.current;
      if (fallbackUri) setMorphedModelUrl(fallbackUri);
      setTimeout(() => scrollViewRef.current?.scrollToEnd({ animated: true }), 300);
    } finally {
    ...
    <ThemedView style={[styles.container, { backgroundColor: colors.background }]}>
      <KeyboardAvoidingView
        ...
        <ScrollView
          ...
          <AnimatedView
            ...
            <View style={styles.heroContent}>
              ...
              {/* Single 3D result viewer: placeholder (ring) until morph, then backend result */}
              {(morphedModelUrl ?? defaultModelUri) ? (
                <AnimatedView 
                  entering={FadeIn.delay(1000).duration(800)}
                  style={[
                    styles.heroModelWrapper,
                    {
                      marginTop: 48,
                    },
                  ]}>
                  <ThemedText
                    style={[
                      styles.heroModelLabel,
                      {
                        color: colors.text,
                        opacity: morphedModelUrl ? 0.9 : 0.5,
                        fontFamily: Platform.select({
                          ios: 'Georgia',
                          android: 'serif',
                          web: 'Georgia, serif',
                    ...
                    {morphedModelUrl
                      ? 'Your 3D result'
                      : 'Your 3D result will appear here — upload a photo and tap Morph'}
                  </ThemedText>
                  <AnimatedView
                    style={[
                      ...
                      {
                        backgroundColor: colors.accent,
                        borderColor: morphedModelUrl ? colors.primary : colors.border,
                        borderWidth: morphedModelUrl ? 2 : 1,
                      },
                    ]}>
                    <ModelViewer
                      modelUrl={morphedModelUrl ?? defaultModelUri!}
        ...
        <Image
              ...
              {/* Take a pic now - Camera capture */}
              <AnimatedView style={[styles.takePicRow, takePicButtonStyle]}>
                <TouchableOpacity
                  style={[
                    styles.takePicButton,
                    {
                      backgroundColor: colors.background,
                      borderColor: colors.border,
                    },
                  ...
                  activeOpacity={0.85}>
                  {isUploading ? (
                    <View style={styles.takePicContent}>
